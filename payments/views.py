...
def gopay_charge(request):
    if request.method == "POST":
        ...
        response = payments.create_payment({
            ...
            "callback": {
                "return_url": request.build_absolute_uri(reverse('payment-succeed')),
                "notification_url": request.build_absolute_uri(reverse('complete')),
            },
        ...
        if response.has_succeed():
            ...
            return JsonResponse({"status": "success", "message": str(response)})
        else:
            print("\nPayment Fail\n")
            print("oops, API returned " + str(response.status_code) + ": " + str(response))
            return JsonResponse({
                "status": "error", 
                "message": f"Payment failed with status {response.status_code}: {str(response)}"
            }, status=400)
